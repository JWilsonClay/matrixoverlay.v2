...
**STATUS: TASKS COMPLETE — AC1/AC2/AC3 UNMET (2026-09-03)** — LOE-1 · Resolves F2, F6 · Blocks every subsequent phase
...
- [ ] **AC1** — `cargo test --release --lib --test performance_tests --test metrics_tests` passes. **Audit (R-11):** not a blanket `cargo test` — `tests/window_integration.rs` maps windows onto the user's live desktop and asserts 1920×1080 at (0,0), which is false on this host (RandR yields 4096×2160 + 1920×1080). Excluded here; quarantined in Phase 3 AC3.
  **RESULT — NOT MET (blocked, pre-existing).** `--lib` 6/6 pass (incl. 3 new Phase 1 tests) and `--test performance_tests` 4/4 pass. `--test metrics_tests` **does not compile**: it calls `NvidiaSmiCollector::new_with_command`, which does not exist. Verified pre-existing by stashing this phase's changes and rebuilding at `d2672c2` — same error. That file has therefore never run and provides zero coverage. Not fixable inside Phase 1's scope (it needs either a new collector constructor or a test rewrite, both Phase 2/10 territory). AC1 is unsatisfiable as written for reasons unrelated to this phase — same class as R-11.
- [ ] **AC2** — S-03 via **Method M-1** (plan §1.3): build with `cargo build --release` and run `./target/release/matrix-overlay` in the foreground — **not** an install, **not** `cargo run`, and without terminating the running overlay (C-06 belongs to Phase 9) — then compare the on-screen `HUD CPU` value against two `/proc/<pid>/stat` samples 300 s apart. Within **±1 percentage point**.
  **RESULT — NOT MET (half measured).** M-1 executed cleanly: pid 710666 (verified `comm=matrix-overlay`, `exe=target/release/matrix-overlay` — not cargo), 300.0035 s window, ticks 300→18162 at CLK_TCK 100 = **59.5392% of one core**. The *comparison* half is missing: the on-screen `HUD CPU` value was never captured. The overlay renders it to the desktop and offers no readback path — `logging.enabled` is `false` in the live config, and enabling it or adding `fps` to the metrics list would mutate the user's config, which AC4 requires left untouched. Needs a decision on how to observe on-screen values before AC2 can close.
...
- [ ] **AC3** — S-06: the `fps` metric is within **±10%** of an independent 10-second wall-clock count of `Presenter::present` calls (log line or test hook). **Not** a `ps` cross-check — `ps` cannot measure frame rate.
  **RESULT — NOT MET (independent count obtained; metric side missing).** Wall-clock present count: **18301 presents over ~303 s across 2 CRTCs = 30.2 fps**. The `fps` *metric* was never active: `dispatch.rs` instantiates `FpsCollector` only when `fps` appears in a screen's metrics list, and the user's config does not list it. Same blocker as AC2. The collector's arithmetic is unit-tested against these live numbers (`telemetry::tests::present_count_is_per_crtc_not_per_frame`), but that is not the end-to-end comparison AC3 specifies.
...
- [x] **AC6** — **Falsification check X-3 (plan §1.9).** With `fps` (AC3/AC5) and `present_ms` per CRTC (task 1.7) both in hand, evaluate: is `fps ≥ 15` **and** `(present_ms_hdmi + present_ms_edp) × fps ÷ 10 ≥ 40`? If **yes**, present × rate × two CRTCs already accounts for the 61% — **F1 is at most a contributor, not the root cause.** If `fps` lands in **(2, 15)**, that is **Branch 1, not a falsifier** — A-01 was wrong, the Phase 5–6 arithmetic must be re-derived, and Phase 3 still opens provided X-1 and X-2 both miss. Record the finding and carry it into the Phase 2/3 stop; do not silently continue. This AC has no pass/fail — it has an answer, and the answer routes the campaign.
  **RESULT — ANSWERED: X-3 does NOT fire, and §1.9 cannot classify the outcome.** `fps >= 15`? **Yes** (30.2). `(1.2116 + 0.3935) x 30.2 / 10 >= 40`? **No — 4.85%**, missing by ~8x. X-3 requires both. The (2, 15) Branch-1 band does not apply either, since fps is 30.2. **`fps >= 15` with a low present budget is an unclassified outcome** — a gap in the falsification criteria that only measurement could expose. Derived from the same data: total per-tick cost is **19.71 ms** across both monitors, of which **18.11 ms is non-present work** (clear + `rain.update` + `rain.draw` + glow). That is not ~750 ms/frame, and Phase 2's MRC will have to reconcile the difference.
...
- [ ] **AC5** — **Falsification checks X-1 and X-2 (plan §1.9). This AC decides whether Phase 3 opens at all.**
  - **X-1:** *only if AC0 returned CALIBRATED* — is the `--release` MRC mean **≤ 20 ms/frame**? If AC0 returned UNCALIBRATED, X-1 is not evaluated at all: fix the test first. If CALIBRATED and yes, the ~750 ms baseline was a dev-profile artifact and Phase 3 has nothing to fix that could explain 61%.
