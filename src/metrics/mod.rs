//! Matrix Overlay Metrics Substrate.
//! Defines core metric types, identifiers, and collection traits.
...
pub use self::manager::{spawn_metrics_thread, SysinfoManager};

#[derive(Debug, Clone)]
pub enum MetricsCommand { UpdateConfig(Config), ForceRefresh }

/// [HARDENED] Unique identifier for metrics with natural naming.
...
impl MetricId {
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            "cpu_usage" => Some(Self::CpuUsage), "ram_usage" => Some(Self::RamUsage),
            "ram_used" => Some(Self::RamUsed), "ram_total" => Some(Self::RamTotal),
            "load_avg" => Some(Self::LoadAvg), "uptime" => Some(Self::Uptime),
            "network_details" => Some(Self::NetworkDetails), "disk_usage" => Some(Self::DiskUsage),
            "cpu_temp" => Some(Self::CpuTemp), "fan_speed" => Some(Self::FanSpeed),
    ...
    pub fn as_str(&self) -> &str {
        match self {
            Self::CpuUsage => "cpu_usage", Self::RamUsage => "ram_usage",
            Self::RamUsed => "ram_used", Self::RamTotal => "ram_total",
            Self::LoadAvg => "load_avg", Self::Uptime => "uptime",
            Self::NetworkDetails => "network_details", Self::DiskUsage => "disk_usage",
            Self::CpuTemp => "cpu_temp", Self::FanSpeed => "fan_speed",
...
#[derive(Debug, Clone)]
pub struct MetricData { pub values: HashMap<MetricId, MetricValue> }
...
#[derive(Debug)]
pub struct SharedMetrics { pub data: MetricData, pub timestamp: Instant, pub day_of_week: String }

impl SharedMetrics {
    pub fn new() -> Self {
        Self { data: MetricData { values: HashMap::new() }, timestamp: Instant::now(), day_of_week: "N/A".to_string() }
    }
}
